    ...
    const settingsToggle = document.getElementById('openSettings');
    ...
    canvas.addEventListener('touchstart', startLongPress, { passive: false });
    canvas.addEventListener('touchend', endLongPress, { passive: false });
    canvas.addEventListener('touchcancel', endLongPress, { passive: false });
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      } else if (recognition && !settingsPanel.classList.contains('open') && !isSpeaking && !isProcessing) {
        setTimeout(() => {
          try {
            recognition.start();
          } catch (e) {
            console.log('Recognition restart after visibility change failed:', e);
          }
        }, 300);
      }
    });
